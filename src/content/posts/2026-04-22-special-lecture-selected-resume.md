...
특히 인상적이었던 건 채용 담당자가 문서를 오래 읽지 않는다는 전제였다. 포트폴리오를 정성껏 만들었다는 사실보다, 첫 장과 첫 프로젝트 블록에서 무엇을 했고 왜 그렇게 했는지가 바로 보여야 한다는 말이 더 현실적으로 들렸다. 이번 특강은 좋은 이력서를 쓰는 방법이라기보다, 개발자 문서를 `어떻게 더 선명하게 만들 것인가`에 가까운 강의였다.
...
대신 실제로 보는 것은 세 가지에 가까웠다. 첫째는 사용 기술의 깊이, 둘째는 시행착오를 어떻게 풀었는지, 셋째는 협업과 코드 품질 감각이다. 같은 `React 사용`이라도 단순히 써 봤다는 말과 상태 관리 문제를 어떻게 나눠 다뤘는지는 완전히 다른 정보라는 뜻이었다.
