...
이 피벗은 단순히 아이디어를 바꾼 수준이 아니었다. 담당 강사가 바뀌면서 팀 프로젝트 과제 방향도 함께 조정됐고, 요구사항 자체가 `Groupware` 기반 앱을 만드는 쪽으로 바뀌었다. 그래서 처음 구상했던 메뉴판 번역 AI보다, 여러 사용자가 일정과 정보, 커뮤니케이션을 함께 다루는 구조가 더 맞다고 판단해 `Mongle`이라는 결혼 준비 협업 앱으로 주제를 전환했다.
...
<img src="/images/posts/mongle/mongle-cover.png" alt="Mongle 로고 시안" style="display:block; width:100%; max-width:420px; margin:1rem auto;" />

<img src="/images/posts/mongle/collaboration-effort-1.png" alt="Discord 채널 분리, 브랜치 규칙, GitHub Issue 관리 등 초기 협업 방식" style="display:block; width:100%; max-width:760px; margin:1rem auto;" />

<img src="/images/posts/mongle/collaboration-effort-2.png" alt="체크리스트 공유와 오프라인 출력까지 포함한 협업 운영 기록" style="display:block; width:100%; max-width:760px; margin:1rem auto;" />
...
## 피벗을 결정할 때 끝까지 붙들었던 질문

아이디어를 바꾸는 일은 늘 아쉽다. 이미 생각해 둔 기능과 화면, 조사한 자료, 팀원 간 합의가 한 번에 흔들릴 수 있기 때문이다. 그런데도 Day 1에서 피벗을 진지하게 검토한 이유는, 처음 떠올린 방향이 팀의 역량과 해커톤의 제한 시간 안에서 정말 설득력 있게 완성될 수 있는지 다시 봤기 때문이다. 좋은 아이디어처럼 보여도 실제로는 차별성이 약하거나, 구현 복잡도에 비해 사용자 효용이 낮을 수 있다. 이 판단을 초기에 하지 않으면 후반에 더 큰 비용을 치르게 된다.

Mongle이 의미 있었던 것은 단순히 웨딩이라는 주제를 택했기 때문이 아니라, 결혼 준비라는 복잡한 의사결정 과정을 AI로 어떻게 줄여줄 수 있을지 문제를 더 구체화했다는 점에 있다. 예산, 일정, 업체 탐색, 질문 응답, 커플 간 동기화처럼 실제 불편이 분산된 지점을 하나의 서비스 흐름으로 묶으려 했다는 점이 프로젝트의 강점이었다.
