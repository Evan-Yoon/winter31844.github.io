...
description: |
  ...
  LLM workflow 최적화로 방향을 다시 정의했다.
  팀 구성과 역할을 구체화하고, MVP 범위도 현실적으로 정리한 날이었다.
thumbnail: ""
tags:
  - tokit
  - llm
...
Tokit 프로젝트의 실제 첫날이었다. 오늘 한 일은 코드를 많이 짠 것보다, **이 프로젝트를 어떤 문제로 정의할지**를 다시 잡는 일이었다.
...
> Tokit은 단순 프롬프트 최적화 도구가 아니라, LLM CLI 앞단에서 작동하는 workflow 최적화 레이어다.

---

## 오늘 정리한 핵심

오늘 기준으로 Tokit을 다시 정의하면 이렇다.
...
이미지 기준으로 보면 오늘 목표는 팀 정보 확정과 프로젝트 기획서 작성이었다. 실제 수행한 작업은 그 목표에서 크게 벗어나지 않았다.

- 프로젝트 팀 구성을 완료했다
- 프로젝트 기획서 작성을 진행했다
- 사용자 페르소나와 유저 플로우를 일부 정리했다
- 통계 기반 인사이트와 유사 서비스 비교 분석을 작성했다
- 기존 서비스와의 차별점을 다시 정리했다
- MVP 기준 목표와 요구사항을 좁혀 정리했다
- 역할 분담 구조를 Task Graph / State / CLI 중심으로 나눴다

진행률 관점에서도 체감이 비슷했다. 팀 구성은 거의 마무리됐고, 기획 문서는 초안이 잡히긴 했지만 완성이라고 보기엔 아직 60% 정도였다.
...
RTK는 가장 직관적이었다. 소개 페이지를 보면 아예 CLI 출력 노이즈를 줄여서 context window를 덜 오염시키는 도구라는 포지션이 분명했다. 테스트 출력이나 `git diff` 같은 결과를 압축해서 더 긴 세션과 더 낮은 비용을 만든다는 설명이 강해서, 이쪽은 입력 이전보다 **출력 이후 필터링**에 더 가깝다고 느꼈다.
...
Headroom은 이름 그대로 context optimization layer 쪽에 가까웠다. LLM 애플리케이션에서 context를 덜 낭비하게 만드는 레이어라는 점은 Tokit과 맞닿아 있었지만, 내가 오늘 정리한 Tokit처럼 **한국어 입력 컴파일, task 이해, state 전달, CLI 흐름**까지 한 번에 묶는 구조와는 결이 조금 달랐다.
...
이쪽은 사용자 터미널 환경과 직접 맞닿는 인터페이스를 개발하고, 실제로 Tokit을 쓸 수 있는 형태로 만드는 역할이다.
