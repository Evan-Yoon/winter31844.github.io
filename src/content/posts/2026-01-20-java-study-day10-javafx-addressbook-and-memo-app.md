...
## 5. GUI 주소록: 콘솔 CRUD가 화면 앱으로 확장됐다
...
1. `ImageView`에 새 이미지를 반영하려면 어떤 흐름이 필요할까?
2. 메모장 예제에서 파일 읽기/쓰기 로직을 컨트롤러 밖으로 뺀 이유는 무엇일까?
3. GUI 주소록에서 메인 창과 신규/수정/삭제 창을 나눈 장점은 무엇일까?
4. `Model.toString()`이 중요한 이유는 무엇일까?
5. 수정 기능은 왜 "조회 -> 값 채우기 -> 수정 -> 저장" 흐름으로 가는 것이 자연스러울까?
6. 파일 기반 CRUD에서 삭제나 수정 시 전체 파일을 다시 쓰는 이유는 무엇일까?
7. 백그라운드 갱신 스레드는 어떤 역할을 하나?
8. day9 계산기와 day10 주소록의 구조 차이는 무엇일까?
