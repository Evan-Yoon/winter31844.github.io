---
title: "[스팸 분류 스터디][2회차] 길이 분포와 워드클라우드로 데이터 읽기"
...
그래서 Day 2에서 얻은 결론은 다음과 같았다.

> 단어 하나보다 단어 조합과 문장 패턴이 더 중요하다.

이 생각이 이후 `CountVectorizer`, `TF-IDF`, `N-gram` 실험으로 자연스럽게 이어졌다.

## 워드클라우드를 보며 느낀 한계도 있었다

워드클라우드는 전체 분위기를 보는 데는 유용하지만, 몇 가지 한계도 분명했다.
