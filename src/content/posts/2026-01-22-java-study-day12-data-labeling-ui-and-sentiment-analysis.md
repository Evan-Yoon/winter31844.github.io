...
## Java 학습 12일차에 작은 AI 프로젝트 형태가 실제로 완성되기 시작했다
...
## 1. JavaFX 데이터 라벨링 앱이 "초안"에서 "실제 도구"에 가까워졌다
...
## 6. day12의 핵심은 "라벨링 후 끝"이 아니라 그 데이터를 학습까지 연결한 것이다
...
## 14. day12가 중요한 이유는 "도구를 연결하는 힘"을 보여 주기 때문이다
...
1. 라벨링 앱에서 중립 라벨을 추가한 이유는 무엇일까?
2. 즉시 저장 방식은 왜 라벨링 도구에서 특히 중요할까?
3. `application.css` 같은 스타일 파일이 데이터 작업 도구에서도 중요한 이유는 무엇일까?
4. 모델이 "좋아요" 같은 표현을 잘못 분류했다는 사실은 어떤 문제를 시사할까?
5. 데이터 증강은 왜 모델 성능 개선에 도움이 될 수 있을까?
6. JavaFX 감정 분석기에서 자바와 파이썬은 각각 어떤 역할을 맡고 있을까?
7. Python 모델이 숫자를 반환하고 Java가 그걸 "긍정/부정"으로 바꾸는 구조의 장점은 무엇일까?
8. day12를 작은 캡스톤 프로젝트라고 볼 수 있는 이유는 무엇일까?
