...
![LSTM 수식 흐름 이해용 도식](/images/posts/intel-nlp-extra2-lstm/lstm-math-flow.svg)
...
gate 값이 전부 1에 가깝다면, 예를 들어 문장에서 주어가 계속 유지되는 상황처럼
기존 문맥을 거의 그대로 들고 간다고 이해하면 된다.

![Forget gate가 모두 0인 경우](/images/posts/intel-nlp-extra2-lstm/lstm-slide-58-58.png)

반대로 gate 값이 전부 0에 가깝다면, 문장이 완전히 다른 주제로 넘어가서
예전 문맥을 거의 버리는 상황에 가깝다.
...
- `"I went to a movie theater ..."`처럼 영화 이야기가 계속 이어지면 관련 메모는 많이 남긴다
- 그런데 뒤에서 `"Now let me talk about today's weather"`처럼 주제가 바뀌면 영화 관련 메모는 크게 줄일 수 있다
- 완전히 버리지는 않더라도 중요도를 낮춰 뒤로 보내는 식이다
...
- 하지만 hidden state는 지금 이 순간 예측에 필요한 내용, 예를 들어 `boring`, `amazing`, `not` 같은 현재 주변 문맥을 더 직접적으로 반영한다
