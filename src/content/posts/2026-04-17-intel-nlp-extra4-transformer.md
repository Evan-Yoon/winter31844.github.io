...
이 발상이 결국 Attention 중심 구조로 이어진다.
...
이걸 처음부터 복잡하게 볼 필요는 없다. 큰 흐름만 먼저 잡으면 된다.
...
즉, Transformer는 순서를 하나씩 읽는 대신, **위치 정보를 입력에 직접 추가**하는 방식으로 순서를 다룬다.
...
![Transformer attention 수식 이해용 도식](/images/posts/intel-nlp-extra4-transformer/transformer-attention-flow.svg)
...
예를 들어 문장 `"The cat sat on the mat"`에서 `sat`를 이해하는 상황을 생각해 보자.
attention이 아래처럼 나왔다면,
...
- `The`는 조금만 참고하고
- `cat`은 많이 참고하고
- `on the mat`도 어느 정도 참고한다

즉, `sat`의 표현을 새로 만들 때 주어인 `cat` 정보가 크게 반영된다는 뜻이다.
...
예를 들어 `"I love ___"` 다음 단어를 생성하는 순간이라면,

- `I`, `love`는 참고할 수 있고
- 아직 생성하지 않은 뒤 단어는 점수를 줘도 안 되므로 mask로 막는다
