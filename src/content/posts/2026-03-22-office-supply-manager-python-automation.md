...
로직은 `current < minimum` 조건 하나로 정리된다. 현재 수량이 최소 기준을 밑돌면 `minimum - current`만큼을 발주 수량으로 계산해서 리스트에 담는다.
...
도메인 지식과 개발 능력이 만나면 진짜 쓸모 있는 도구가 나온다. 이번 작은 스크립트에서도 그걸 다시 느꼈다. 비품 관리처럼 작아 보이는 업무도 방식만 통일되면 재고 부족, 중복 구매, 요청 누락 같은 운영 피로를 꽤 줄일 수 있다.
