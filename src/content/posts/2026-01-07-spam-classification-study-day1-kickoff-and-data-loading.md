...
2026년 1월 7일, 스팸 분류 스터디를 시작했다. 이 스터디는 개발 경험이 있는 최규철님이 직접 개설해 주셨고, 목표도 비교적 분명했다.
...
스터디 자료 폴더를 열어 보니 `Dockerfile`, `docker-compose.yml`, `requirements.txt`까지 같이 들어 있었다. 즉 각자 로컬 환경에 맞춰 대충 실행하는 것이 아니라, 가능하면 비슷한 환경에서 주피터 랩을 열고 같은 결과를 보자는 의도가 분명했다.
...
첫날 데이터 점검에서 가장 먼저 걸린 것은 불필요한 컬럼이었다. `isnull().sum()`을 확인해 보니 아래처럼 거의 비어 있는 열이 세 개나 있었다.
